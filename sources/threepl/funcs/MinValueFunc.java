...
/**
 * An inbuilt function to get the minimum numeric value of a target
 * variable. This has 1 argument, which is the variable.
 */
public class MinValueFunc extends InbuiltFunc implements Constant {
    ...
    public Val getVal (NodeList args) {
        ...
        if (args.size() != 1)
            throw new ExEx("minvalue() must have a single argument", loc);
        ...
        if (size > 63)
            throw new ExEx("minvalue() - argument size greater than 63 bits", loc);
        ...
        case ENUM:
            ...
            break;
        case NONE:
            throw new ExEx("minvalue() argument is not a primitive type ", loc);
        default:
            throw new ExEx("minvalue() argument cannot be type " + ptype.typename(), loc);
        }
        tdev.setVal(lval);
