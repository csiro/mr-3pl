...
/**
 * An inbuilt procedure to read from a registered output RAM block.
 * This has 3 input arguments and no output arguments.
 *
 ...
 */
public class RmemoryReadProc extends InbuiltProc implements Constant, TDEConstants {

    /**
     * Construct the inbuilt procedure rmemoryread().
     * This is called exactly once by method threepl.exec.InbuiltProcs().
     */
    public RmemoryReadProc () {
    ...
    public void execute (
        ...
        if (inargs.size() != 3)
            throw new ExEx("rmemoryread() must have 3 input arguments", loc);
        if (outargs.size() != 0)
            throw new ExEx("rmemoryread() must have no output arguments", loc);
        ...
        // identifier
        Ref     ref = inargs.getRef(0, "rmemoryread()");
        if (ref.getMode() != Mode.RMEMORY)
            throw new ExEx("rmemoryread() 1st argument is not an rmemory variable", loc);
        ...
        var.memRead (port, clockvar, addr, queues, start_del, "rmemoryread()", loc);
