 ...
 */
public class FileIsFileFunc extends InbuiltFunc implements Constant {
    ...
    public Val getVal (NodeList args) {
        ...
        if (args.size() != 1)
            throw new ExEx("fileisfile() must have 1 argument", loc);
        Val     val = args.getVal(0);
        if (val.getMode() != Mode.IMMEDIATE)
            throw new ExEx("fileisfile() argument not immediate mode", loc);
        File    file = null;
        boolean ret;
        ...
        case FILE:
            ...
            if (of == null)
                throw new ExEx("fileisfile() file argument has not been opened", loc);
            file = of.getFile();
            break;
        default:
            throw new ExEx("fileisfile() argument not a file or a string", loc);
        }
        
        try {
            ret = file.isFile();
        } catch (SecurityException e) {
            throw new ExEx("fileisfile() - access problem", loc);
        }
