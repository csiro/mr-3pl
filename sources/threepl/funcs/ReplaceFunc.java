 ...
 * The first occurence selected by the 2nd argument, if any, will
 * be replaced.
 */
public class ReplaceFunc extends InbuiltFunc implements Constant {
    ...
    public Val getVal (NodeList args) {
        ...
        if (args.size() != 3)
            throw new ExEx("replace() must have 3 arguments", loc);
        ...
        String  s;
        try {
            s = s1.replaceFirst(s2, s3);
        } catch (PatternSyntaxException e) {
            throw new ExEx("substring() - bad regular expression", loc);
        }
