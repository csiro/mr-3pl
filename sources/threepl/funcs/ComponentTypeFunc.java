 ...
 */
public class ComponentTypeFunc extends InbuiltFunc implements Constant {
    ...
    public Val getVal (NodeList args) {
        ...
        if (args.size() != 2)
            throw new ExEx("componenttype() must have two arguments", loc);

        Ref ref = args.getRef(0, "componentpointer() ");
        int index = (int)args.getVal(1).getSingleIval(loc);
        if (index < 0)
            throw new ExEx("componenttype() second argument is negative", loc);
        if (index >= ref.numWords())
            throw new ExEx("componenttype() second argument is too large", loc);
        int i = ref.getWordSpec().getWord(index);
        switch (ref.getMode()) {
        case VALUE:
            Value v = (Value)ref.getVar();
            return(new Val(v.getWordRef(i, loc).getType(), loc));
        case STATIC:
            Static s = (Static)ref.getVar();
            return(new Val(s.getWordRef(i, loc).getType(), loc));
        case QUEUE:
            Queue p = (Queue)ref.getVar();
            return(new Val(p.getWordRef(i, loc).getType(), loc));
        default:
            throw new ExEx("componenttype() first argument " + ref.getMode().name() + " mode not allowed", loc);
        }
