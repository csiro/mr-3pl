...
\chapter{Inbuilt Procedures}
    ...
    \section{assert - assert a value mode variable for a clock cycle}\label{sec:ipassert}
        ...
        The following would not work in the same way -
        \begin{lstlisting}[gobble=8, language=threepl]
           value(v, "log");
...
%        on 3-state nets, e.g. BUFT, BUFE, PULLUP, PULLDOWN, KEEPER.
    ...
    \section{cmemory - create a combinatorial output memory}\label{sec:ipcmemory}
        ...
        {\bf Description:} \\
        Create a combinatorial output RAM.
        
        {\em name} is an identifier for the memory. The variable identifier can
        ...
        {\em ports} is the number of ports.

        {\em atype} is the address type. This can be any target type with the
        constraint that the total number of bits must not exceed 9.

        {\em dtype} is the data type. This can be any target type.
        
        {\em init} is an optional initialisation array. The initialisation data is
        a one-dimensional variable or array which is no larger than the depth of
        the memory. It may be smaller in which case the memory is zero padded. The
        types and structure of the initialisation variable or structured constant
        after the first dimension must match the memory data type. Immediate
        constants are padded to fit target elements but must not exceed the target
        width. If the initialisation argument is omitted the memory will be
        initialised to all zero.

        An initial value may also be set later by calling inbuilt procedure {\bf
        attributes()}
        
        Additional trailing arguments may be passed to set attributes. These
        must be of the form attr=value.
        
        This procedure may create a module, procedure or function
    ...
    \section{cmemorywrite - write to a combinatorial output memory}\label{sec:ipcmemorywrite}
        ...
        param  & mode     & type  & \\
        \hline \hline
        m    & M             &  -  & memory being written   \\ \hline
        ...
        {\em m} is the memory mode variable identifier.
    ...
    \section{divrem - combined quotient and remainder}\label{sec:ipdivrem}
        ...
        \begin{lstlisting}[gobble=8, language=threepl]
           procedure
           divrem (value(quot), value(rem) <- value(num), value(denom)) {}
        \end{lstlisting}
        ...
        {\bf Input parameters:} \\            
        {\bf num} is the numerator. \\
        {\bf denom} is the denominator. \\
        ...
        Note: this procedure uses cascaded adders and subtractors and
        will have a long data path delay. However, the code is optimised
        for some cases where one operand is constant - 
        for division of 0 (return 0:0), division by 1 (return n:0) and
        division by power of 2 (quotient by shifting, remainder by masking).
    ...
    \section{rmemory - create a registered output memory}\label{sec:iprmemory}
        ...
        {\bf Description:} \\
        Create a registered output RAM.
        ...
        {\em atype} is the address type. This can be any target type with the
        constraint that the total number of bits must not exceed the maximum
        block RAM address width for the FPGA type. For Virtex, Virtex E and
        Spartan 2 this is 12 bits. For Virtex2, Virtex2 PRO and Spartan 3
        this is 14 bits.
        ...
        \autorefph{sec:ipattributes} with key "init".         
        A number of constraint strings or string arrays may be appended. Each
        constraint will be applied to each component block RAM, in each case
        with "INST {\em name} " prepended where {\em name} is the netlist
        identifier of each block RAM.
