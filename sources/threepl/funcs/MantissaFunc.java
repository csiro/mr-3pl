...
/**
 * An inbuilt function to get the mantissa of a value of primitive type
 ...
 * error.  The mantissa is returned as a target value type "uint:n"
 * where 'n' is the number of bits in the mantissa.
 */
public class MantissaFunc extends InbuiltFunc implements Constant {
    ...
    public Val getVal (NodeList args) {
        ...
        if (args.size() != 1)
            throw new ExEx("mantissa() must have a single argument", loc);

        Val val = args.getVal(0);
        if (!val.isPrimitive())
            throw new ExEx("mantissa() argument not a primitive type", loc);
        if (val.getPrimType() != Ptype.FLOAT)
            throw new ExEx("mantissa() argument not type \"float\"", loc);        
        ...
        case IMMEDIATE:
            ...
            return(new Val(l & 0xfffffffffffffL, loc));
        default:
            throw new ExEx("mantissa() argument not allowed target mode", loc);        
        }
        ...
        Type        otype = new Type("uint:" + mwidth, loc);
        WordSpec    ows = otype.getWordSpec(null, loc);
        TDEVar      otdev = tdelist.signal("MANTISSA", ows, loc);
        tdelist.connect(tmp_tdev, itdev);
        tdelist.connect(otdev, tmp_tdev.getWord(2, loc));
