...
/**
 * An inbuilt procedure to write to a registered output RAM block.
 ...
 */
public class RmemoryWriteProc extends InbuiltProc implements Constant, TDEConstants {

    /**
     * Construct the inbuilt procedure rmemorywrite().
     * This is called exactly once by method threepl.exec.InbuiltProcs().
     */
    public RmemoryWriteProc () {
    ...
    /**
     * Execute the procedure rmemorywrite(). This does not generate in-line
     * executable code.
    ...
    public void execute (
        ...
        if (inargs.size() != 4)
            throw new ExEx("rmemorywrite() must have 4 input arguments", loc);
        if (outargs.size() != 0)
            throw new ExEx("rmemorywrite() must have no output arguments", loc);
        ...
        if (ref.getMode() != Mode.RMEMORY)
            throw new ExEx("rmemorywrite() 1st argument is not an rmemory variable", loc);
        ...
        var.memWrite (port, clockvar, addr, data, queues, start_del, "rmemorywrite() -", loc);
