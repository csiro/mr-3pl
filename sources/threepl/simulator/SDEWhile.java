...
/**
 * The SDEWhile class implements the TDEType.WHILE TDE in the threepl
 * simulator.
 * 
 * @version $Revision: 6003 $
 * @author Andrew Tulloh
 */
public class SDEWhile extends SDE {
     ...
     */
    protected SDEWhile(Sim sim, TDE tde) {
        super(sim, tde);
    }
   ...
   public boolean behaviour(int oIndex, double time, boolean force) {
      ...
      // contin    = input(2)
      // clock     = input(3)
      // resetInput = input(4) - optional
      //
      // startb    = output(0)
      // finish    = output(1)
      //
      // startb = test && (start || contin)
      // finish = q (=d = !test && (start || contin))
      //
      switch (oIndex) {
      case 0:
	 putOutput(0, getInputBit(1, time, force) &&
	              (getInputBit(0, time, force) || getInputBit(2, time, force)));
	 break;
      case 1:
	 putOutput(1, q);
	 break;
     ...
     * Required for SDEWhile because its flip flop d input
   ...
   public void inputBehaviour (double time, boolean force) {   
      d = !getInputBit(1, time, force) && (getInputBit(0, time, force) || getInputBit(2, time, force));
      if (resetConnected)
