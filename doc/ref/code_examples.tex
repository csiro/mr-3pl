...
\chapter{Code Examples}
    ...
    \section{function generation using look-up tables}\label{sec:fglut}
        ...
        As written this code will loop continuously with {\bf addr} starting
        at 0 and wrapping back through 0 each time around the loop. As a minor
        flaw rmemoryout() will be undefined the first time through the loop
        since it is used in the same clock cycle as the first read is performed.
        To fix this an initial read could be performed, but now the loop would
        have to be explicit -
        \begin{lstlisting}[gobble=8, language=threepl]
            .
            .
