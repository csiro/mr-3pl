...
/**
 * An inbuilt procedure to declare an immediate logical variable.
 */
public class LogProc extends InbuiltProc implements Constant {

    /**
     * Construct the inbuilt procedure log().
    ...
    /**
     * Execute the variable declaration procedure log().
    ...
    public ArrayList<Var> execute (
        ...
        if ((inargs.size() < 1) || (inargs.size() > 2))
            throw new ExEx("log() must have 1 or 2 input arguments", loc);
        if (outargs.size() != 0)
            throw new ExEx("log() cannot have output arguments", loc);
        if (dim_des != null)
            throw new ExEx("log() as a parameter cannot have a compound constant argument", loc);

        ArrayList<Ident>        idents = resolveIdents(inargs, names, "log");
        ...
        for (Ident id : idents) {
            Immediate ivar = new Immediate(id, new Type(Ptype.LOG, 0), init, is_input, is_output, loc);
