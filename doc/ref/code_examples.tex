...
\chapter{Code Examples}
    ...
    \section{traversing a memory}\label{sec:traverse}
        ...
        Note that we could pass arguments to rmemory() by key=vaue rather than
        by position. These key=vaue arguments can be in any order. This is more
        verbose but more readable -
        \begin{lstlisting}[gobble=8, language=threepl]
           rmemory(mem, ports=1, atype="uint:8", dtype="uint:8");
        \end{lstlisting}
        A mixture of the two is allowed as long as no positional arguments
        follow key=value arguments.
    ...
    \section{function generation using look-up tables}\label{sec:fglut}
        ...
        to a queue -
        \begin{lstlisting}[gobble=8, language=threepl]
            // Immediate code to initialise the table.
            type(at, "uint:8");     // address type
            type(dt, "int:12");     // data type
            var(mi, "[256]int");    // memory initialisation array
            for (int(i) ; i<256 ; i++)
                mi[i] = floor(2047.0 * sin(2 * i * math_pi / 256.0));

            rmemory(m, ports=1, atype=at, dtype=dt, init=mi);

            static(maddr, at);
            queue(sinstream, dt, depth=16);

            sync {
                rmemoryread(m, port=0, addr=maddr);
                sinstream <<- rmemoryout(m, port=0);
                maddr++;
            }
        \end{lstlisting}
        Here the address and data types are explicitly created as
        types. Strings specifying types, as always, can be used instead, e.g.
        \begin{lstlisting}[gobble=8, language=threepl]
            rmemory(m, ports=1, atype="uint:8", dtype="int:12", init=mi);
        \end{lstlisting}
        
        If you prefer a cryptic style.
        \begin{lstlisting}[gobble=8, language=threepl]
            rmemory(m, 1, at, dt, mi);
        \end{lstlisting}
        
        The sin values are generated by immediate code and used as an
        initialisation array for the block RAM (rmemory). The addresses are
        unsigned in the example as only the first quadrant is covered. For more
        quandrants the address input could be signed. The rmemory has an 8 bit
        address to hold 256 values and the stored values are 12 bit signed. As
        written this code will loop continuously with {\bf addr} starting at 0
        and wrapping back through 0 each time around the loop.
        
        As a minor flaw rmemoryout() will be undefined the first time through
        the loop since it is used in the same clock cycle as the first read is
        performed. To fix this an initial read could be performed, but now the
        loop would have to be explicit (sync while) rather than implicit
        (the previous top-level sync) -
        \begin{lstlisting}[gobble=8, language=threepl]
            .
            .
            .
        ...
        \end{lstlisting}
        which would avoid implicitly multiplexing the address input between
        'addr' and 0.
        
        A simpler solution is to initialise the output latch of the block
        RAM -
        \begin{lstlisting}[gobble=8, language=threepl]
            rmemory(m, 1, "uint:8", "uint:12", init=mi, init0=mi[0]);
        \end{lstlisting}

        The following code takes the sin stream and drives an external MCP4822
        DAC to produce an analog sine wave. The data stream to the DAC is much
        ...
        Where rmemory() data is not a primitive type an equivalent immediate
        mode type for initialisation data can be created by builtin function
        ...
        \begin{lstlisting}[gobble=8, language=threepl]
            ...
            type(sti, targtoimtype(st));    // equivalent immediate type
            var(initd, "[512]sti");           // data initialisation array
            for (int(i) ; i<512 ; i++) {
                initd[0].f1 = ......;
                initd[0].f2 = ......;
            }
            var(initl, sti);                // output latch initialisation
            initl.f1 = 123;
            initl.f2 = 58;
            rmemory(m, ports=1, atype="uint:7", dtype=st, init=initd, init0=initl);
            .
            .
            .
        ...
        Note that both initialisation data and output latch initialisation
        values can be passed as compound constants, e.g.
        \begin{lstlisting}[gobble=8, language=threepl]
            rmemory(m, ports=1, atype="uint:3", dtype=st, init=initd, init0={123, 58});
        \end{lstlisting}
        For data it is nearly always more convernient to compute a data array of
        values.
        
        If initialisation values are passed via builtin procedure
        attributes(), e.g.
        \begin{lstlisting}[gobble=8, language=threepl]
            rmemory(m, ports=1, atype="uint:3", dtype=st);
            attributes(m, init=initd, init0=initl);
        \end{lstlisting}
        
        then compound constant arguments cannot be used as attributes() cannot
        determine the data type to parse the constant.
