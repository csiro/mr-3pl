...
/**
 * An inbuilt function to get the previous value of an enumerated type value.
 * This has 1 argument which must be of enumerated type.
 * The type returned is the same as that of the argument.
 * If the argument is immediate fatal error occurs if there is no previous value.
 * If the argument is mode value, selectvalue, static or queue a value
 * with an ordinal of 0 is returned if there is no previous value.
 */
public class PrevFunc extends InbuiltFunc implements Constant, TDEConstants {
    ...
    public Val getVal (NodeList args) {
        ...
        if (args.size() != 1)
            throw new ExEx("prev() must have 1 argument", loc);

        
        Val     val = args.getVal(0);
        if (val.getPrimType() != Ptype.ENUM)
            throw new ExEx("prev() argument is not an enumerated type", loc);
        Mode    mode = val.getMode();
        Type    type = val.getType();
        Ptype   ptype = type.getPrimType();
        if (ptype != Ptype.ENUM)
            throw new ExEx("prev() argument is not an enumerated type (" + ptype.typename() + ")", loc);
        switch (mode) {
        case IMMEDIATE:
            long ord = val.enumOrd(loc);
            return(new Val(type.prevEnumOrd(ord, loc), type, loc));
        case VALUE:
        case SELECTVALUE:
        case STATIC:
        case QUEUE:
            ...
            return(retval);
        default:
            throw new ExEx("prev() argument mode " + mode.modename() + " not allowed", loc);
        }
    }
}
