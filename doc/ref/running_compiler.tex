...
\label{chap:run3pl}
    ...
    \section{Environment variables}
        ...
        then the post-processing can be supressed by one of the following -
        \blist
        \item   Give the -s option to 3PL.
        \item   Include the following in the 3PL source -\\
                \begin{lstlisting}[gobble=12, language=threepl]
                    directive("skipPostProc", true)
                \end{lstlisting}
        \item   Define environment variable {\bf THREEPL\_SKIP\_POSTPROC}.
        \item   Remove or comment out the postmodule section in the platform
                FPGA file, e.g. xc3s.3pl.
        \blend
        ...
        Runs after completion of 3PL netlist file generation, constraint file
        generation, report file generation etc. when 3PL would normally exit.
        SETTING UP postprocessing
        rationale - is immediate code only - any target code produces fatal error
        link to  module page
        error messages not quie right - will fix later
        Is initially intended to run XIlinx place-and-route tools following
        3PL code generation, but could be used for other or additional purpsese.
	...
	The {\bf design\_name} is derived from the source file name argument by
	...
	also observes EDIF identifier name restrictions. 
        ...
        \blist
        \item   a Xilinx EDIF netlist file called {\bf design\_name.edn}
        \item   either a Xilinx ISE constraint file, {\bf design\_name.ncf}, or a Vivado
                constraint file, {\bf design\_name.xdc}, where {\bf design\_name} is the design name.
        \blend
        It may optionally produce - \\
        \blist
        \item   a detailed report file {\bf design\_name.rpt} (-r option) \\
        \item   an intermediate code file {\bf design\_name.icl} (-t option) \\
        \item   a list of equivalent net identifiers {\bf design\_name.net} (-n option).
        \blend
        
        FPGA boards which provide a CPU interface, either by a serial port (e.g.
        papilio or AVNET boards) of a parallel shared bus (e.g. zynq) are supported
        by 3PL library code. A shared interface, {\bf include/xilinx/cpu.3pl} is
        common to both. For serial ports the underlying logic is contained in {\bf
        include/xilinx/uart.cpu}. For the zynq the underlying bus interface is
        created by {\bf include/xilinx/zynq\_axi.3pl}. When included these
        interfaces generate a file {\bf design\_name.info} which specifies addresses, data
        types and data widths which are used by utilities to generate C or Python
        interface code in the CPU.
        
        The {\bf -I} option specifies a ":"-separated list of directories to be
        searched for files to be included by {\bf source} and {\bf module}
        statements. There may be more than one {\bf -I} and they may precede or
        follow the source file name. The "-I" may be separated from its list by
        spaces, i.e. the list is the following argument. Directories are searched
        in the order - current directory, {\bf -I} option lists in order, then
        finally the 3PL library directory. Therefore, source/module files may be
        preempted by including alternative versions earlier in the directory search
        path.
        ...
        \item   -d - display the interactive netlist schematic window after
                outputing an EDIF netlist - broken! see note below
        ...
        \item   -n - print a list of netlist net identifiers and their
                equivalent signal names to a file {\bf design\_name.net}
        ...
        \item   -r - generate a report to file {\bf design\_name.rpt}
%        \item   -S - run the simulator instead of producing output code
        ...
        The -r option causes a detailed report to be produced in file {\bf
        design\_name.rpt}.
        ...
        The -d option should display a window in which the EDIF netlist can be
        examined schematically. Entering a signal name results in the source and
        sinks for that signal being displayed. Clicking on input or output
        pins of displayed elements expands the nelist. Unfortunately this is not
        very useful and hence has been little used. As a result it appears to
        have atrophied over a decade or so since its last use and this has not
        been noticed. It may be fixed in the future but is a low priority. It
        would be more useful if it displayed the intermediate code.
