...
/**
 * An inbuilt procedure to assert the output value mode variable of type "log"
 * in the clock cycle in which this procedure executes.
...
public class AssertProc extends InbuiltProc implements Constant, TDEConstants {
    ...
    /**
     * Construct the inbuilt procedure assert().
    ...
    public void execute (
        ...
        if (inargs.size() > 1)
            throw new ExEx("assert() must have 0 or 1 input arguments", loc);
        if (outargs.size() != 1)
            throw new ExEx("assert() must have one output argument", loc);
