...
\usepackage{inchmargins,fancyhdr,amsmath,graphicx,listings,color,crtitle}
\pagestyle{fancy}




\newcommand{\htitle}{An Introduction to the 3PL Language}
\newcommand{\hauthor}{Paul Dunn}
\newcommand{\hdate}{May 2008}
\newcommand{\hreport}{Open Report CMSE-2008-150}
\newcommand{\hhead}{}
\newcommand{\hfoot}{Open Report CMSE-2008-150}
...
{\sffamily\bfseries\Large Contents} \\
...
\section{Preface}
\addcontentsline{atc}{atoc}{\protect\numberline{\thesection}Preface}
    ...
    3PL is a language intended to allow parallel algorithms to be coded for implementation
    in an FPGA. A language which compiled directly into an FPGA configuration was considered
    insufficient on its own since it is often necessary to run an algorithm which in turn
    generates the target algorithm. For example an extreme case is a batcher sorting network
    where the batcher algorithm must be executed to generate the network topology. In simpler
    cases it is an advantage to parameterise the target algorithm which again implies some
    sort of preliminary processing. This could be accomplished by using a general purpose
    language such as C, Java or Python to pre-process the target algorithm, but it was
    decided that it was preferable to combine all of this in one language. Thus 3PL
    is actually a sequential language similar in most respects to C, but it has additional
    statement types that generate FPGA target code. This might have been accomplished by
    adding appropriate classes to Java or Python but it was felt that a new language
    tailored to the task would be preferable.    
    ...
    \item   it should provide a mechanism whereby systolic pipelines can be constructed
    which are data driven.
    \item   the control of execution threads must co-exist with data-driven execution.
    \item   it should be concise.
    \blend
    

\section{Variable modes}
\addcontentsline{atc}{atoc}{\protect\numberline{\thesection}Variable modes}
...
\addcontentsline{atc}{atoc}{\protect\numberline{\thesection}Static mode variables}
...
\addcontentsline{atc}{atoc}{\protect\numberline{\thesection}Input and output mode variables}
    ...
    In the code above the \var{loc} attribute must either be a string or an array or
    strings. These are used to generate FPGA pad location constraints.
    Note that array dimensions precede the array type.
...
\addcontentsline{atc}{atoc}{\protect\numberline{\thesection}Value mode variables}
   ...
   Consider the following code fragment -
    ...
    pointer variables but these are not discussed here as they are seldom used
    in application code).
...
\addcontentsline{atc}{atoc}{\protect\numberline{\thesection}Parallel and serial blocks}
    ...
    again starting simultaneously. Thus in the first case the two elements repeat
    independently whereas in the second case they repeat in step.

\section{Target loops}
\addcontentsline{atc}{atoc}{\protect\numberline{\thesection}Target loops}
...
\addcontentsline{atc}{atoc}{\protect\numberline{\thesection}Immediate execution}
...
\addcontentsline{atc}{atoc}{\protect\numberline{\thesection}Target statements}
    ...
    The target statement equivalent of an (immediate) {\bf if ()} statement
...
\addcontentsline{atc}{atoc}{\protect\numberline{\thesection}Queue mode variables}
    ...
    \end{lstlisting}
    which examines queues \var{q2} and \var{p3} and reads whichever has the largest
    datum,  assigning it to queue \var{q1}. The sync ensures that \var{q2} and
    \var{p3} are available for reading and \var{q1} is available for writing
    ...
    because \var{p3} is not yet available, the second pipeline path being longer.
...
\addcontentsline{atc}{atoc}{\protect\numberline{\thesection}Target exceptions}
...
\addcontentsline{atc}{atoc}{\protect\numberline{\thesection}combinatorial output memory}
    ...
    Target fixed point types could also be used, rather than integer}.
    A variation is to use a two-port cmemory and allow table entries to be
    overwritten from an external CPU when required. The Zynq platform
    provides this facility as do many simple FPGA platforms with a serial
    interface from a USB port. In these cases the above code might then be
    written as -
    \begin{lstlisting}[language=threepl]
        class(cpu, cpu_api());
        str(dtype, "uint:8");
        ...
        .
    \end{lstlisting}
    The first line creates a CPU interface class.
    The class procedure \kwd{write()} allows a CPU coupled to the FPGA
    to write to a queue variable in the FPGA. The input arguments \var{"WD"} and
    \var{"WA"} are string identifiers for the bus interface to the FPGA.
...
\pagebreak
    ...
    In this last example if the input queue \var{aq} transmits all zeros the
    output data stream \var{dq} will be at the same rate, otherwise the output
...
\addcontentsline{atc}{atoc}{\protect\numberline{\thesection}registered output memory}
...
\addcontentsline{atc}{atoc}{\protect\numberline{\thesection}Clocks}
    ...
    For a static variable the current clock domain when the first assignment statement
    in which the variable appears (either LHS or RHS) is encountered establishes the
    clock domain for the variable. Any other assignments to or evaluations of that
    ...
    clock domain is established as the current clock domain when the first write to the
    queue is encountered and all subsequent writes must be in the same clock domain.
    Similarly the read clock domain is established as the current clock domain when the
    first read of the queue is encountered and all subsequent reads must be in the same
    clock domain. If the read and write clock domains are the same, which is the usual
    case, the queue is called {\bf synchronous} and the queue buffer depth will be 2 by
    default. If the read and write clock domains differ the queue is called {\bf
    asynchronous} and the default buffer depth is 16. Resynchronisation of queue data
    between the two clock domains is handled automatically by the asynchronous queue.
...
\addcontentsline{atc}{atoc}{\protect\numberline{\thesection}Modules}
    ...
    \end{lstlisting}
    Here when either of the modules reads its input queue, popping a datum, the queue
    will appear to be no longer available (empty) until the other module also executes a
    read. If both reads occur simultaneously then the next queue datum will immediately
    be available to both modules. Thus queue reads in multiple sinks will all get the
    same datum, subsequent reads waiting until all modules have been satisfied.
    
    Where there are multiple read statements within one module -
    \blist
    \item   The reads are independent of each other.
    \item   The first queue read to occur will extract a datum as described above.
            Any subsequent queue read by any these statements will wait until
            all other queue sinks (modules) have read that datum.
    \item   If one or more queue reads can execute simultaneously, i.e. their
            input and output queues are all available, then they will execute
            in the same clock cycle and will read the same datum
    \item   If queue reads occur at different times then each will pop
            different (successive) data.
    \blend
    
    It is common to use separate statements to read different fields where
    the data type is a {\bf struct} and in this case it is usual to enclose these
    in a {\bf sync\verb'{}'} to ensure they execute in the same clock cycle.
    ...
    access the scope in which it is enclosed - in the case above \var{q}, \var{q1}
    ...
    occur when both \var{q2} and \var{q} are available. If \var{q1} and \var{q2}
    ...
    will not see \var{q} become available again until after the second read has
    ...
    and \var{q2} being assigned the datum from \var{q}. If however \var{q} and
    \var{q1} were available but \var{q2} was not, the assignment to \var{q1} would
    take place and the datum would be popped from \var{q}. When \var{q2} became
    available it would read the next datum from \var{q}. Another solution might be 
    \begin{lstlisting}[language=threepl]
    queue({q, q1, q2}, t1);
...
\addcontentsline{atc}{atoc}{\protect\numberline{\thesection}Modules, procedures, functions and classes}
    ...
    The un-named module can access the scope of the function in which it is contained.
    Note that the assignments to variable \var{d} are not in a par block. Since we are
    at the top level of a module each assignment statement will execute in parallel in
    its own infinite loop at clock rate\footnote{The code generator will eliminate the
    implied infinite loops and simply control the enables to the register flip-flops.
    How and when these are asserted depends on how execution in the FPGA is started. It
    can be started by an external signal or it can start spontaneously after
    configuration - see target-execution-start in the manual index. A static variable
    can have its 'continuous' attribute set to true in which case the enables will be
    tied high.}.
...
\addcontentsline{atc}{atoc}{\protect\numberline{\thesection}program structure and scope}
...
\addcontentsline{atc}{atoc}{\protect\numberline{\thesection}Selectvalue mode}
...
\addcontentsline{atc}{atoc}{\protect\numberline{\thesection}Priority mode}
    ...
    ignored. An additional output is available via the inbuilt function
    {\bf prielse()}, e.g. {\bf prielse(arb)}, which is true if none of the inputs
    is true.


\section{More on types}
\addcontentsline{atc}{atoc}{\protect\numberline{\thesection}More on types}
...
\addcontentsline{atc}{atoc}{\protect\numberline{\thesection}Inbuilt modules, procedures and functions}
...
\section{Petri nets}
\addcontentsline{atc}{atoc}{\protect\numberline{\thesection}Petri nets}
...
\pagebreak    
    ...
    \end{lstlisting}
    Here there are places \var{q1}, \var{q2}, \var{p3} and \var{p4}. Each of these
    ...
    their tokens removed (are cleared) and places \var{p3} and \var{p4} have tokens
