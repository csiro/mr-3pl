...
\section{Common Xilinx Procedures Library File - xilinx/common.3pl}
    ...
    \subsection{library procedures}
        ...
        \subsubsection{iopins - create FPGA pin specifications}\label{sec:lpiopins}
            ...
            Each attribute value must be either a single value which will be added
            to each pin map.
            ...
            \end{lstlisting}
            which will create a variable {\bf led\_pins} in the current scope which
            is an array of 4 strings. The I/O standard "lvcmos33" will be
            established for each of the four pins (inputs or outputs) and a pullup
            resistor is specified for each. For two of the pins separate timing name
            attributes have been added.
            
            The following example would produce an arror -
            \begin{lstlisting}[gobble=12, language=threepl]
               static(led, "bits:4");
            ...
            \end{lstlisting}
            In this example the four pins initially have attribute pullup true
            then on the next call to iopins() two of them have the pullup
            attribute changed to false. The error occurs when these four pins are
            passed to output() as the pullup attribute is now not the same for
            all pins. If the pullup attribute was passed directly to the call to
            output() then the pullup attribute values associated with the pins
            via iopins() would be ignored, e.g.
            \begin{lstlisting}[gobble=12, language=threepl]
               static(led, "bits:4");
               iopins(led_pins, "[4]str", {"A1", "B1", "A2", "C2"}, pullup=true);
               iopins(, "[2]str", {"B1", "A2"}, pullup=false);
