...
\section{variable creation}
    ...
    \hyperref[sec:ipident]{ident} & I P &  extract an identifier from an argument & \refp{sec:ipident} \\
...
\section{string}
    ...
    \hyperref[sec:ifstrpos]{strpos} & I F & find leftmost substring position & \refp{sec:ifstrpos} \\
...
\section{external memory}
    ...
    \begin{tabular}{l l l l}
    \hyperref[sec:lcqdr2ram]{qdr2\_ram} & L C & QDR2 RAM class & \refp{sec:lcqdr2ram} \\
    \hyperref[sec:lcqdr2ramread]{qdr2\_ram.read} & L CM & read from QDR2 RAM & \refp{sec:lcqdr2ramread} \\
    \hyperref[sec:lcqdr2ramread]{qdr2\_ram.readp} & L CM & read from QDR2 RAM with priority & \refp{sec:lcqdr2ramread} \\
    \hyperref[sec:lcqdr2ramwrite]{qdr2\_ram.write} & L CM & write to QDR2 RAM & \refp{sec:lcqdr2ramwrite} \\
    \hyperref[sec:lcqdr2ramwrite]{qdr2\_ram.writep} & L CM & write to QDR2 RAM with priority & \refp{sec:lcqdr2ramwrite} \\
    ...
    \hyperref[sec:lczbtram]{zbt\_ram} & L C & ZBT RAM class & \refp{sec:lczbtram} \\
    \hyperref[sec:lczbtramread]{zbt\_ram.read} & L CM & read from ZBT RAM & \refp{sec:lmextzbtramread} \\
    \hyperref[sec:lczbtramread]{zbt\_ram.readp} & L CM & read from ZBT RAM with priority & \refp{sec:lmextzbtramread} \\
    \hyperref[sec:lczbtramwrite]{zbt\_ram.write} & L CM & write to ZBT RAM & \refp{sec:lmextzbtramwrite} \\
    \hyperref[sec:lczbtramwrite]{zbt\_ram.writep} & L CM & write to ZBT RAM with priority & \refp{sec:lmextzbtramwrite} \\
    \end{tabular}
...
\section{CPU interface}
    ...
    \hyperref[sec:lpcpurdqueue]{cpu\_api.read\_queue} & L CP & a CPU read from a queue variable (UART, ZYNQ) & \refp{sec:lpcpurdqueue} \\
    ...
    \hyperref[sec:lpcpuwrstat]{cpu\_api.write\_static} & LS CP & a CPU-writable static variable (UART, ZYNQ) & \refp{sec:lpcpuwrstat} \\
    \hyperref[sec:lpinterrupt]{cpu\_api.interrupt} & L CP & interrupt the CPU (UART, ZYNQ) & \refp{sec:lpinterrupt} \\

    \hyperref[sec:lmzynqrddma]{dma\_read} & I CM & DMA transfer from memory to FPGA (ZYNQ) & \refp{sec:lmzynqrddma} \\
    \hyperref[sec:lmzynqwrdma]{dma\_write} & I CM & DMA transfer from FPGA to memory (ZYNQ) & \refp{sec:lmzynqwrdma} \\
    \end{tabular}
...
%    \hyperref[sec:ifprev]{prev} & I F & get the previous value of an enum & \refp{sec:ifprev} \\
    ...
    \hyperref[sec:lfsvnversion]{svn\_version} & L F & get the current subversion version number & \refp{sec:lfsvnversion} \\
...
%    \hyperref[sec:ipincludedir]{includedir} & I P & add directory for include path & \refp{sec:ipincludedir} \\
    ...
    \hyperref[sec:ipimport]{import} & L P & import a 'core' & \refp{sec:ipimport} \\
