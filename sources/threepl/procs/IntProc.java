...
/**
 * An inbuilt procedure to declare an immediate integer variable.
 */
public class IntProc extends InbuiltProc implements Constant {

    /**
     * Construct the inbuilt procedure int().
    ...
    public ArrayList<Var> execute (
        ...
        if ((inargs.size() < 1) || (inargs.size() > 2))
            throw new ExEx("int() must have 1 or 2 input arguments", loc);
        if (outargs.size() != 0)
            throw new ExEx("int() cannot have output arguments", loc);
        if (dim_des != null)
            throw new ExEx("int() as a parameter cannot have a compound constant argument", loc);
        ...
        for (Ident id : idents) {
            Immediate ivar = new Immediate(id, new Type(Ptype.INT, 0), init, is_input, is_output, loc);
