...
/**
 * An inbuilt procedure to declare an immediate "class" variable.
 */
public class ClassProc extends InbuiltProc implements Constant {

    /**
     * Construct the inbuilt procedure class().
    ...
    /**
     * Execute the variable declaration procedure class().
    ...
    public ArrayList<Var> execute (
        ...
        if ((inargs.size() < 1) || (inargs.size() > 2))
            throw new ExEx("class() must have 1 or 2 input arguments", loc);
        if (outargs.size() != 0)
            throw new ExEx("class() cannot have output arguments", loc);
        if (dim_des != null)
            throw new ExEx("class() as a parameter cannot have a compound constant argument", loc);

        Val                     init = null;
        ArrayList<Ident>        idents = resolveIdents(inargs, names, "int");
        ...
        if (initialise && (inargs.size() == 2)) {
            if (idents.size() != 1)
                throw new ExEx("class() cannot have an initial value for multiple variables", loc);
            init = inargs.getVal(1);
        }
        
        // construct the variables
        for (Ident id : idents) {
            Immediate ivar = new Immediate(id, new Type(Ptype.CLASS, 0), init, is_input, is_output, loc);
