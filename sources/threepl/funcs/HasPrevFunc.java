...
/**
 * An inbuilt function to determine if there is a previous ordinal of an
 * enumerated type value. This has 1 argument which must be
 * of enumerated type. The value returned is type log.
 */
public class HasPrevFunc extends InbuiltFunc implements Constant {
    ...
    public Val getVal (NodeList args) {
        ...
        if (args.size() != 1)
            throw new ExEx("hasprev() must have 1 argument", loc);
        ...
        if (ptype != Ptype.ENUM)
            throw new ExEx("hasprev() argument is not an enumerated type", loc);
        switch (mode) {
        case IMMEDIATE:
            long    ord = val.enumOrd(loc);
            Type    t = val.getType();
            return(new Val(t.hasPrevEnumOrd(ord, loc), loc));
        case VALUE:
        case SELECTVALUE:
        case STATIC:
        case QUEUE:
            WordSpec    lws = new WordSpec(1, Ptype.LOG);
            long        l = type.firstEnumOrd();
            ...
            return(retval);
        default:
            throw new ExEx("hasprev() argument mode " + mode.modename() + " not allowed", loc);
        }
    }
}
