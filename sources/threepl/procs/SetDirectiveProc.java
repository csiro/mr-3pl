...
/**
 * DEPRECATED - NOW CALLED DIRECTIVE(), OTHERWISE IDENTICAL.
 ...
 */
public class SetDirectiveProc extends InbuiltProc implements Constant {

    /**
     * Construct the inbuilt procedure setdirective().
     * This is called exactly once by method threepl.exec.InbuiltProcs().
     */
    public SetDirectiveProc () {
    ...
    public void execute (
        ...
        if ((inargs.size() < 1) || (inargs.size() > 2))
            throw new ExEx("setdirective() must have one or two input arguments", loc);
        if (outargs.size() != 0)
            throw new ExEx("setdirective() cannot have output arguments", loc);
        ...
        case GLOBAL:    // DEPRECATED - there is only global scope now!
            break;
        default:
            throw new ExEx("setdirective() - directive identifier cannot have a scope context", loc);
        }
        ...
        if (inargs.size() == 2) {
            ...
            if (valval.isTarget())
                throw new ExEx("setdirective() value cannot be a target variable or expression", loc);
            if (valval.numWords() > 1)
                throw new ExEx("setdirective() value cannot be an array", loc);
            ...
            case LOG:
                break;
            default:
                throw new ExEx("setdirective() value type " + valval.getPrimType().typename() + " not allowed", loc);
            }
        } else
