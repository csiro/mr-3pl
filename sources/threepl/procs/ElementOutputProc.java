...
/**
 * An inbuilt procedure to connect an output signal to a port of
 ...
 */
public class ElementOutputProc extends InbuiltProc implements Constant, TDEConstants {

    /**
     * Construct the inbuilt procedure elementoutput().
     * This is called exactly once by method threepl.exec.InbuiltProcs().
     */
    public ElementOutputProc () {
    ...
    /**
     * Execute the procedure elementoutput().
    ...
    public void execute (
        ...
        if ((inargs.size() != 2) && (inargs.size() != 3))
            throw new ExEx("elementoutput() - must have 2 or 3 input arguments", loc);
        if (outargs.size() != 1)
            throw new ExEx("elementoutput() - must have 1 output argument", loc);
        
        if (outargs.isNull(0) || (outargs.getRef(0, "elementoutput()") == null))
        ...
        if (inargs.getNode(0) != null) {
            ...
            if ((val.getMode() != Mode.IMMEDIATE) || (val.getPrimType() != Ptype.STR))
                throw new ExEx("elementoutput() - 1st input argument must be a string", loc);
        ...
        val = inargs.getVal(1);
        if ((val.getMode() != Mode.IMMEDIATE) || (val.getPrimType() != Ptype.STR))
            throw new ExEx("elementoutput() - 2nd input argument must be a string", loc);
        ...
        if (inargs.size() == 3) {
            ...
            if ((val.getMode() != Mode.IMMEDIATE))
                throw new ExEx("elementoutput() - 3rd input argument must be an int", loc);
            arrayformat = (int)val.getSingleIval(loc);
            if ((arrayformat < 0) || (arrayformat > 2))
                throw new ExEx("elementoutput() - 3rd input argument must be in range 0 to 2", loc);
            arrayformat = (int)val.getSingleIval(loc);
        }
        
        // signal
        ref = outargs.getRef(0, "elementoutput()");
        switch (ref.getMode()) {
        case VALUE:
            WordSpec    ws = ref.getWordSpec();
            tdev = tdelist.signal("OUT", ws, loc);
            ref.assignTo(new Val(null, Mode.VALUE, tdev, loc), loc);
            break;
        case CLOCK:
            Clock cvar = (Clock)ref.getVar();
            tdev = cvar.getClkSig();
            //cvar.setClkSourced();
            break;
        case IMMEDIATE:
            val = outargs.getVal(0);
            if (val.getPrimType() == Ptype.STR) {
                ...
                throw new ExEx("elementoutput() - output argument immediate mode but not a string", loc);
            break;
        default:
            throw new ExEx("elementoutput() - output argument not allowed mode", loc);
        }
        ...
        if (arrayformat == 0) {
            if (width == 1)
                ...
                throw new ExEx("elementoutput() - array input with no width or array format provided", loc);
        }
        if ((width == 0) && (arrayformat == 2))
            throw new ExEx("elementoutput() - array format code 2 cannot be used with a single target value", loc);

        tdelist.addPort(bname, portName, 1, tdev, width, arrayformat);
    }
}
