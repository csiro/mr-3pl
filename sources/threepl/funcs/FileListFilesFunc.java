...
/**
 * An inbuilt function to list files.
 * This has one argument which is the file path string or open file
 * of the directory to be listed.
 * The function returns an array of strings.
 */
public class FileListFilesFunc extends InbuiltFunc implements Constant {
    ...
    public Val getVal (NodeList args) {
        ...
        if (args.size() != 1)
            throw new ExEx("filelistfiles() must have 1 argument", loc);
        Val     val = args.getVal(0);
        if (val.getMode() != Mode.IMMEDIATE)
            throw new ExEx("filelistfiles() argument not immediate mode", loc);
        if (val.getPrimType() != Ptype.STR)
            throw new ExEx("filelistfiles() argument not a string", loc);
        File    file = null;
        File[]  files = null;
        ...
        case FILE:
            ...
            if (of == null)
                throw new ExEx("filelistfiles() file argument has not been opened", loc);
            file = of.getFile();
            break;
        default:
            throw new ExEx("filelistfiles() argument not a file or a string", loc);
        }
        ...
        try {
            ...
            throw new ExEx("filelistfiles() - access problem", loc);
        }
